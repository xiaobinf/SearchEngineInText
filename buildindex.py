...
class BuildIndex:
	...
	def process_files(self):
		...
		for file in self.filenames:
			...
			# [\W]+ 匹配不是字母数字下划线中文的字符  re.compile('')创建模式对象
			pattern = re.compile('[\W_]+')
			file_to_terms[file] = open('corpus/'+file, 'r').read().lower();
			# 用空格替换 文件中的\W_字符
	...
	def fullIndex(self):
		...
		for filename in indie_indices.keys():
			self.tf[filename] = {}
			# indie_indices[filename].keys() {filename: {word: [pos1, pos2, ...]}, ...}所有单词
