...
"""Do this:
	Calculate a tf-idf score for every unique term in the collection, for each document.
	 As in, find all unique terms, and for each document, got through 
	each unique term and calculate a tf-idf score for it in the doc. 
	You can do this already with the generateScore function. Doc becomes array of scores.
...
q = Query(['pg11.txt','pg5200.txt'])
print(q.invertedIndex)
with open('invertIndex.txt','w') as f:
	f.write(str(q.invertedIndex))
# print(q.invertedIndex.keys())
print(q.one_word_query('paleness'))
print(q.one_word_query('white'))
print(q.free_text_query('paleness is white'))
print(q.phrase_query('paleness white'))
